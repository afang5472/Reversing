...
def calc2(stack_var1, var, x=0):
    ...
    '''
    if x==1:
        s2 = res1 + 0xC40BF11DDFCD22E 
        s3 = 0x59D39E717F7 * var 
        s4 = (s2-s3) % (1<<56)
        print hex(s2)
        print hex(s3)
        print hex(s4)
        print hex(stack_var1)
        print hex(res1)
        raw_input("res1")
    '''
...
def algorithm(our_input):
    ...
    for i in range(0,8):
        ...
        if i == 7:
            ...
            raw_input("check temp_var2!")

        for l in range(0,10):
            
            for m in range(0,4):

                temp = calc2(temp_var2[2*m], const)
                if i == 7 and l == 8:
                    print hex(const)
                if i == 7 and l == 9:
                    print "par1 : " + hex(showlong(temp_var2[2*m]))
                    print "par2 : " + hex(showlong(const))
                    print "res: "   + hex(showlong(temp))
                temp_var2[2*m] = temp_var2[2*m+1]
                temp_var2[2*m + 1] = calc2(temp,temp_var2[2*m])
                if i == 7 and l == 9:
                    print "par1: " + hex(showlong(temp))
                    print "par2: " + hex(showlong(temp_var2[2*m]))
                    print "res: "  + hex(showlong(temp_var2[2*m+1]))
                const += 1
        for n in range(0,8):
            for o in range(0,8):
                our_output[8*(o^n)+o] = chr(temp_var2[n]>>(o*7)&0x7f)
        print "conster: " + hex(showlong(const))
